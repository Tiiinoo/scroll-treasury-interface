...
def _get_signers_from_calldata(wallet_id: str, address: str, chain: str, conn) -> int:
    ...
    Bypasses the Safe Transaction Service API entirely. For each outgoing tx
    missing signers:
      1. eth_getTransactionByHash  → extract packed signatures from calldata
      2. eth_getTransactionReceipt → read message_hash from ExecutionSuccess
         event topics[1] (txHash is indexed — this is exactly what signers
         signed, so nonce computation is not needed)
      3. ecrecover every 65-byte signature against that message hash
    ...
    # keccak256("ExecutionSuccess(bytes32,uint256)") — txHash is indexed → topics[1]
    EXEC_SUCCESS_TOPIC = "0x" + keccak(b"ExecutionSuccess(bytes32,uint256)").hex()
    ...
    # Deduplicate — multiple ERC20 rows share the same tx_hash
    unique_hashes = list(dict.fromkeys(r["tx_hash"] for r in rows))
    logger.info("%s/%s: calldata signer recovery for %d tx(s)", wallet_id, chain, len(unique_hashes))
    ...
    for tx_hash in unique_hashes:
        try:
            # ── 1. Get packed signatures from execTransaction calldata ─────
            ...
            if (not input_data
                ...
                continue  # not an execTransaction — no multisig signing pattern

            # tx_from is the address that submitted execTransaction to the chain.
            # Needed for v=1 (approved-hash) signatures where signer == msg.sender.
            tx_from = result.get("from", "")
            try:
                tx_from = to_checksum_address(tx_from) if tx_from else ""
            except Exception:
                pass

            raw_input = bytes.fromhex(input_data[2:])
            (*_, signatures) = eth_abi.decode(
                ["address", "uint256", "bytes", "uint8",
                 "uint256", "uint256", "uint256",
                 "address", "address", "bytes"],
                raw_input[4:],  # skip 4-byte selector
            )
            if not signatures:
                continue

            # ── 2. Get the exact message hash from ExecutionSuccess event ──
            # The Safe emits ExecutionSuccess(bytes32 indexed txHash, uint256 payment).
            # txHash is indexed → it lives in topics[1], not in data.
            # This IS the EIP-712 message hash that every signer signed, so we
            # never have to compute domain separators or nonces ourselves.
            rcpt_resp = _proxy("eth_getTransactionReceipt", {"txhash": tx_hash})
            time.sleep(0.22)
            if not rcpt_resp:
                continue
            ...
            for log_entry in receipt.get("logs", []):
                ...
                if (log_entry.get("address", "").lower() == address.lower()
                        and len(topics) >= 2
                        and topics[0].lower() == EXEC_SUCCESS_TOPIC.lower()):
                    message_hash = bytes.fromhex(topics[1][2:])  # strip 0x
                    break

            if not message_hash:
                logger.debug("%s/%s: no ExecutionSuccess log for %s", wallet_id, chain, tx_hash[:10])
                continue

            # ── 3. Recover signers from packed 65-byte signatures ──────────
            # Packed format: r(32) + s(32) + v(1) per signer.
            # Use n // 65 count — do NOT use (len - 64) which off-by-ones on 2+ signers.
            recovered = []
            n_sigs = len(signatures) // 65
            for i in range(n_sigs):
                sig_slice = signatures[i * 65: i * 65 + 65]
                r_int = int.from_bytes(sig_slice[:32], "big")
                s_int = int.from_bytes(sig_slice[32:64], "big")
                v = sig_slice[64]

                if v == 1:
                    # Approved hash: signer is msg.sender (the tx submitter)
                    if tx_from:
                        recovered.append(tx_from)
                elif v in (27, 28):
                    # Standard ECDSA: sign message_hash directly
                    v_norm = v - 27
                    ...
                    # eth_sign: Safe uses ecrecover(keccak256("\x19Ethereum Signed Message:\n32" + message_hash), ...)
                    v_norm = v - 31
                    hash_to_sign = keccak(b"\x19Ethereum Signed Message:\n32" + message_hash)
                    try:
                        sig_obj = eth_keys_lib.Signature(vrs=(v_norm, r_int, s_int))
                        pub_key = sig_obj.recover_public_key_from_msg_hash(hash_to_sign)
                        recovered.append(pub_key.to_checksum_address())
                        ...
                        pass
                # v=0 (contract signature) — not recoverable from calldata alone
